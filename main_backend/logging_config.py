...
def setup_logging():
	...
	log_type = os.getenv('THEGOODBROWSER_LOGGING_LEVEL', 'info').lower()
	...
	class BrowserUseFormatter(logging.Formatter):
		def format(self, record):
			if record.name.startswith('thegoodbrowser.'):
	...
	browser_use_logger = logging.getLogger('thegoodbrowser')
	browser_use_logger.propagate = False  # Don't propagate to root logger
	browser_use_logger.addHandler(console)
	browser_use_logger.setLevel(root.level)  # Set same level as root logger

	logger = logging.getLogger('thegoodbrowser')
	logger.info('theGoodBrowser logging setup complete with level %s', log_type)
